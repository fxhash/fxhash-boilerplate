...
const ghDownloader = new Downloader({
  github: {
    auth: "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",
  },
})
...
const changesManager = {
  fxlens: {
    getVersion: async () => {
      const { data } = await axios.get(
        ...
        {
          headers: {
            Authorization: "Bearer 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",
          },
        }
      )
      return data[0].sha
  ...
  snippet: {
    getVersion: async () => {
      const { data } = await axios.get(
        `https://api.github.com/repos/fxhash/fxhash-boilerplate/commits?path=lib/scripts/install.js&per_page=1`,
        {
          headers: {
            Authorization: "Bearer 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",
          },
